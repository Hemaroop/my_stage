...
class StageNode
{
  ...
  std::vector<Stg::ModelBlobfinder *> blobfndrmodels;
  ...
  std::vector<ros::Publisher> blob_pubs_;
...
StageNode::ghfunc(Stg::Model* mod, StageNode* node)
{
  ...
  if (dynamic_cast<Stg::ModelBlobfinder *>(mod))
    node->blobfndrmodels.push_back(dynamic_cast<Stg::ModelBlobfinder *>(mod));
}
